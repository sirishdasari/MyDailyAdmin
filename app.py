...
@mcp.tool()
def update_profile(user_id: str, display_name: str = "", avatar_url: str = "", timezone: str = "", week_start_day: int | None = None, theme: str = "", default_project_id: str = "", onboarding_complete: bool | None = None):
    """Update a user's profile."""
    return services()["profiles"].update_profile(user_id, display_name, avatar_url, timezone, week_start_day, theme, default_project_id, onboarding_complete)


# ---------------- Projects ----------------

@mcp.tool()
def list_projects(user_id: str, include_archived: bool = False, limit: int = 100):
    """List projects belonging to an Appwrite Auth user."""
    return services()["projects"].list_projects(user_id, include_archived, limit)
...
@mcp.tool()
def create_project(user_id: str, name: str, color: str = "", icon: str = "", view_style: str = "", parent_id: str = "", is_favorite: bool = False, is_archived: bool = False, order: float = 0):
    """Create a project."""
    return services()["projects"].create_project(user_id, name, color, icon, view_style, parent_id, is_favorite, is_archived, order)

@mcp.tool()
def update_project(user_id: str, project_id: str, name: str = "", color: str = "", icon: str = "", view_style: str = "", parent_id: str = "", is_favorite: bool | None = None, is_archived: bool | None = None, order: float | None = None):
    """Update a project."""
    return services()["projects"].update_project(user_id, project_id, name, color, icon, view_style, parent_id, is_favorite, is_archived, order)
...
@mcp.tool()
def list_project_tasks(user_id: str, include_archived_projects: bool = False, is_completed: bool | None = None, limit_per_project: int = 100):
    """List projects with matching tasks grouped under each project."""
    projects = services()["projects"].list_projects(user_id=user_id, include_archived=include_archived_projects, limit=100)
    project_ids = [project.get("$id") or project.get("id") for project in projects]
    tasks_by_project = services()["tasks"].list_tasks_by_projects(user_id=user_id, project_ids=[p for p in project_ids if p], is_completed=is_completed, limit_per_project=limit_per_project)
    project_documents = [{**project, "tasks": tasks_by_project.get(project.get("$id") or project.get("id"), [])} for project in projects]
    return {"documentType": "projectTasks", "userId": user_id, "filters": {"includeArchivedProjects": include_archived_projects, "isCompleted": is_completed, "limitPerProject": limit_per_project}, "projectCount": len(project_documents), "taskCount": sum(len(p["tasks"]) for p in project_documents), "projects": project_documents}
...
if __name__ == "__main__":
    ...
    allowed_hosts = [x.strip() for x in os.environ.get("MCP_ALLOWED_HOSTS", "").split(",") if x.strip()]
    allowed_origins = [x.strip() for x in os.environ.get("MCP_ALLOWED_ORIGINS", "").split(",") if x.strip()]
    kwargs = {"transport": "streamable-http", "host": "0.0.0.0", "port": port, "streamable_http_path": "/mcp", "stateless_http": True, "json_response": True}
    if allowed_hosts or allowed_origins:
        kwargs["transport_security"] = TransportSecuritySettings(allowed_hosts=allowed_hosts or None, allowed_origins=allowed_origins or None)
    mcp.run(**kwargs)
